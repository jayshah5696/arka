...
## 2024-11-20 - Palette: Colorize terminal output
* **What**: Replaced `print` and `click.echo` with `click.secho` in `src/arka/cli.py` and `src/arka/pipeline/runner.py`. Added `# DX:` comments to document the purpose of the change.
* **Why**: To remove developer friction by making it easier to quickly scan terminal output for success, failure, and progress indicators using colored text (e.g., green for success, red for errors, yellow for skipped stages, cyan for progress).
* **Before**: All CLI output was plain uncolored text.
* **After**: Terminal output highlights errors in red, successes in green, progress in cyan, and skipped steps in yellow.
