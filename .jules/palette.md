...
## 2024-11-20 - Palette: Colorize CLI summary and errors for better readability
* **What**: Updated `_print_summary`, `_load_config`, `_validate_config`, and `_run_pipeline` in `src/arka/cli.py` to replace standard `print()` and `click.echo()` calls with `click.secho()`, injecting colors (`fg="green"` for success, `fg="red"` for errors, `bold=True` for headers).
* **Why**: To remove developer friction by drawing visual attention immediately to whether a pipeline run completed successfully or crashed. It previously blended in with standard terminal logging making it hard to parse errors at a glance.
* **Before**: All pipeline summary metrics, configuration errors, and execution crash reports were printed in plain text using the default terminal color.
* **After**: Pipeline successes pop in green, failures/crashes in red, and standard metrics flow cleanly.
