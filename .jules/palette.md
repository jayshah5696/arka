...
## 2024-11-20 - Palette: Added colored output to pipeline summary using `click.secho`
* **What**: Replaced standard `print` and `click.echo` statements with `click.secho` in `src/arka/cli.py` to add color formatting. Specifically highlighting dataset output paths in green, and pipeline/stage execution errors in bold red.
* **Why**: To remove developer friction by improving terminal readability. Using color allows developers to instantly distinguish between successful operations (e.g., green dataset paths) and critical failures (e.g., red stage errors) without needing to parse walls of monochrome text.
* **Before**: All CLI outputs were plain text, making it difficult to spot fatal errors mixed in with standard pipeline yield summaries.
* **After**: Important CLI outputs are properly colored: `fg="green"` for success paths, `fg="red"` for errors and failures.
