...
## 2024-11-20 - Palette: Add color-coded terminal output for CLI errors and success
* **What**: Replaced `click.echo` with `click.secho` in CLI output methods, adding `fg="red"` for errors and `fg="green"` for success messages.
* **Why**: To remove developer friction by making critical output stand out visually, reducing cognitive load when parsing terminal output.
* **Before**: All CLI text was plain white.
* **After**: Errors are clearly highlighted in red, and valid config success is green.
