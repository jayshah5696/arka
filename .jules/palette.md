...
## 2024-11-20 - Palette: Colorized CLI Summary
* **What**: Updated `_print_summary` in `src/arka/cli.py` to use `click.secho()` with colors instead of `print()`.
* **Why**: To remove developer friction by making the final summary easy to scan, highlighting successes in green, errors/dropped records in red, and output paths in blue.
* **Before**: The CLI summary printed as plain text making it hard to quickly spot errors or find the output path.
* **After**: The CLI summary uses colors to effectively highlight important information.
