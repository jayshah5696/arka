...
## 2026-07-14 - Palette: Colorize error and success output in CLI\n* **What**: Replaced `click.echo` and plain `print` with `click.secho` to apply red and green color highlighting for error and success messages respectively within `src/arka/cli.py`.\n* **Why**: Developer friction exists when navigating long CLI outputs, making warnings and fatal stage failures hard to quickly identify. Colorization guides developer focus immediately to the core issues during debugging and gives positive reinforcement when configurations load correctly.\n* **Before**: Terminal output used standard uncolored text printing for warnings, missing file exceptions, validation feedback, and execution summary errors.\n* **After**: Success logs show in green, while execution summary stage drops, missing config files, and config validation problems now render in red.
