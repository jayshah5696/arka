...
def _print_summary(
    ...
    # DX: Add color to CLI output to make pipeline status and errors immediately visible
    ...
    click.secho(f"\n--- Pipeline Summary ({status}) ---", fg=color, bold=True)
    click.echo(f"Run ID: {report.get('run_id', run_id)}")
    if duration_secs is not None:
        if duration_secs < 60:
            click.echo(f"Duration: {duration_secs:.1f}s")
        else:
            mins = int(duration_secs // 60)
            secs = duration_secs % 60
            click.echo(f"Duration: {mins}m {secs:.1f}s")
    click.echo(f"Final Count: {report.get('final_count', 0)} records")
    ...
    if dataset_path:
        click.secho(f"\nDataset Output: {dataset_path}", fg="cyan")
    ...
    for stage in report.get("stage_yields", []):
        ...
        if status == "failed":
            ...
            stage_color = "green" if status == "completed" else "yellow"
            click.secho(
...
def _validate_config(
    ...
    try:
        ...
        click.secho(f"Error: Configuration is invalid: {exc}", fg="red", err=True)
        sys.exit(1)
