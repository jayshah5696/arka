...
def _print_summary(
    ...
    if error_info:
        ...
        # DX: Use click.secho with fg='red' to make the fatal error stand out in the summary
        click.secho(
    ...
    for stage in report.get("stage_yields", []):
        ...
        if status == "failed":
            ...
            # DX: Use click.secho with fg='red' to highlight failed stage yields and lost records
            click.secho(
...
def _load_config(config_path: Path) -> ResolvedConfig:
    ...
    try:
        ...
        # DX: Colorize missing file errors for better visibility
        click.secho(
            f"Error: Configuration file not found at {config_path}", fg="red", err=True
        )
        sys.exit(1)
    except ConfigValidationError as exc:
        # DX: Colorize configuration validation errors for better visibility
...
def _validate_config(
    ...
    try:
        ...
        # DX: Colorize validation failures to make them immediately obvious
        click.secho(f"Configuration is invalid: {exc}", fg="red", err=True)
        sys.exit(1)

    # DX: Use green for success messages to provide clear, positive feedback
...
def _run_pipeline(
    *,
    ...
    if error_to_report is not None:
        ...
        # DX: Colorize the fatal pipeline execution error for immediate visibility
        click.secho(
