...
def _load_config(config_path: Path) -> ResolvedConfig:
    ...
    try:
        ...
        # DX: Use click.secho with fg="red" to highlight errors
        click.secho(f"Error: Configuration file not found at {config_path}", fg="red", err=True)
        sys.exit(1)
...
def _validate_config(
    ...
    # DX: Use click.secho with fg="green" to highlight success
...
def _run_pipeline(
    *,
    ...
    if error_to_report is not None:
        ...
        # DX: Use click.secho with fg="red" to highlight errors
        click.secho(f"\nError: Pipeline execution failed - {error_to_report}", fg="red", err=True)
        sys.exit(1)
