...
def _print_summary(
    ...
    # DX: Added color formatting to the pipeline summary to make it easier for developers to quickly identify success, errors, and output paths.
    click.secho(
        f"\n--- Pipeline Summary ({report.get('status', 'unknown')}) ---", bold=True
    )
    ...
    if dataset_path:
        click.secho(f"\nDataset Output: {dataset_path}", fg="blue")
