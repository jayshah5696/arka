...
def _load_config(config_path: Path) -> ResolvedConfig:
    ...
    try:
        ...
        # DX: Colorize errors for better visibility
        click.secho(
...
def _validate_config(
    ...
    try:
        ...
        # DX: Colorize invalid config errors
        click.secho(f"Configuration is invalid: {exc}", fg="red", err=True)
        sys.exit(1)

    # DX: Colorize valid config success message
...
def _run_pipeline(
    *,
    ...
    if error_to_report is not None:
        ...
        # DX: Colorize pipeline execution failures
        click.secho(
